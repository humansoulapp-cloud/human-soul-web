...
import { journeyById } from "@/lib/content";
...
export default function JourneyDetailPage() {
  ...
  if (!journey) {
    return (
      <div className="text-center py-12 space-y-4">
        <p className="text-sm text-[var(--text-secondary)]">Journey not found.</p>
        <Link href="/journeys" className="text-xs text-[var(--brand-primary)] underline">
  ...
  const currentDay = journey.days[activeDayIndex] || journey.days[0];
  ...
  const handleSaveReflection = async (e: React.FormEvent) => {
    ...
    if (!error) {
      ...
      setAnswer("");
      setTimeout(() => {
        if (activeDayIndex < journey.days.length - 1) {
  ...
  return (
    <div className="space-y-8 max-w-2xl mx-auto">
      ...
      <div className="bg-[var(--bg-surface-secondary)] border border-[var(--border-subtle)] rounded-3xl p-6 sm:p-8 space-y-4">
        <div className="flex items-center justify-between text-xs text-[var(--text-secondary)]">
          ...
          <div className="flex items-center gap-1">
            ...
            <span>{journey.timeRequired}</span>
          </div>
        </div>
      ...
      <div className="space-y-3">
        <div className="flex items-center justify-between">
          <h2 className="font-serif-editorial text-2xl text-[var(--text-primary)]">
            Day {currentDay.day} of {journey.days.length}: {currentDay.title}
          </h2>
        </div>
      ...
      <div className="bg-[var(--bg-surface)] border border-[var(--border-subtle)] rounded-3xl p-6 space-y-6">
        ...
        <form onSubmit={handleSaveReflection} className="space-y-4 pt-4 border-t border-[var(--border-subtle)]">
          {successMsg && (
